...
#                                             [r31, r32, r33]])
        ...
        def show(self, height=750, width=1200, view='camera'):
            ...
            cloud_pc = self.publish(
                height=height, width=width, view=view, show=True)
            warnings.filterwarnings('ignore')
            plotObj = cloud_pc.plot(initial_point_size=0.001, backend="pythreejs",
                                    polylines=self.lines, height=height, width=width)
        ...
        def save_to_file(self, height=750, width=1200, view='camera'):
            ...
            cloud_pc.to_file(
                rm_config['rmstudio_path']+'lib/html/rm_dashboard/models/point_cloud.ply')
            df = pd.DataFrame(self.lines)
            df.to_json(rm_config['rmstudio_path'] +
