...
class Cloud_Segmentation:
    ...
    def get_cloud_from_mask(self, mask, depth, height, vertical=False):
        ...
        if len(y) < 500:
            ...
            perm = np.random.permutation(
                int((np.vstack(depth_idxs).shape)[1]/4))
    ...
    def segment_rings_outer(self, cloud, depth, ir, ring_dims, leaf_size, tolerance, vertical=False):
        ...
        masks = []
        circles = cv2.HoughCircles(ir, cv2.HOUGH_GRADIENT, 1, 40, param1=15, param2=30,
                                   minRadius=ring_OR-ring_tolerance, maxRadius=ring_OR+ring_tolerance)
        try:
        ...
        for circle in circles:
            ...
            cv2.circle(mask, (circle[0], circle[1]),
                       circle[2], (255, 255, 255), ring_thickness)
            masks.append(mask)
        ...
        ring_objs = []
        for mask in masks:
            ring_obj = self.get_cloud_from_mask(
                mask, depth, ring_dims[2], vertical=vertical)
