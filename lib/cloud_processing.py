...
class Cloud_Processing:
    ...
    def sort_clouds_height(self, clouds, high_to_low=True):
        """
    ...
    def sort_clouds_size(self, clouds, large_to_small=True):
        """
    ...
    def sort_clouds_distance(self, clouds, pose, close_to_far=True):
        """
                Parameters
        ----------
        clouds: list of k [n,3] ndarrays corrisponding to clouds.
        ...
        """
        poses = self.get_cloud_pose(clouds)
        clouds_and_poses = zip(clouds, poses)

        def sort_fun(x):
            return self.get_disance_between_poses(pose, x[1])
    ...
    def compare_dimensions(self, object_cloud, model_dims, tolerance=0., transform=None, check_height=False, smaller_than=False):
        ...
        if transform is None:
            ...
            object_cloud_transformed = self.transform_points(
